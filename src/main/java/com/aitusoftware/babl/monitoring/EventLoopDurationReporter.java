...
public final class EventLoopDurationReporter
{
    ...
    public EventLoopDurationReporter(final LongConsumer durationReceiver)
    {
        this.durationReceiver = durationReceiver;
    }

    public void eventLoopStart(final long currentTimeMs)
    {
        eventLoopStartMs = currentTimeMs;
    }

    public void eventLoopComplete(final long currentTimeMs)
    {
        if (((currentTimeMs - lastResetMs) & RESET_INTERVAL_MASK) != 0)
        {
            maxEventLoopDurationMs = 0;
            lastResetMs = currentTimeMs;
        }
        maxEventLoopDurationMs = Math.max(maxEventLoopDurationMs, currentTimeMs - eventLoopStartMs);
